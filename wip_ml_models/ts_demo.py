...
class EncryptedLR:
    ...
    def backward(self, enc_x, enc_out, enc_y):
        ...
        self._delta_w = enc_x * out_minus_y if self._delta_w is None else self._delta_w + enc_x * out_minus_y
    ...
    def plain_mse(self, x_test, y_test):
        w = torch.tensor(self.weight.decrypt() if isinstance(self.weight, ts.CKKSVector) else self.weight)
        b = torch.tensor(self.bias.decrypt()[0] if isinstance(self.bias, ts.CKKSVector) else self.bias)
