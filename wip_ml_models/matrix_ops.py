...
def dot_product(HE, enc_vec1, enc_vec2):
    ...
    # Compute the product of each pair and rescale
    for v1, v2 in zip(enc_vec1, enc_vec2):
        print("mult")
        product = v1 * v2
        product = ~(product)
        products.append(product)

    # Sum the products and rescale again if needed
    result = products[0]
    for p in products[1:]:
        result += p
    ...
    for i in range(rows1):
        for j in range(cols2):
            dot_prod = dot_product(
                HE, enc_mat1[i], [row[j] for row in enc_mat2])
            result[i][j] = dot_prod
...
enc_X = encrypt_matrix(HE, X)
enc_y = encrypt_matrix(HE, y)
...
def gd(enc_X, enc_y, lr=0.01, iter=10):
    print("Starting gradient descent...")

    theta = np.random.rand(len(enc_X[0]), 1)

    enc_theta = encrypt_matrix(HE, theta)

    print(f"Initial theta: {theta}")
    print(f"Initial theta shape: {len(enc_theta)}x{len(enc_theta[0])}")

    for _ in range(iter):
        print(f'Iteration {_+1}')
        y_pred = matrix_product(HE, enc_X, enc_theta)
        error = matrix_subtraction(HE, enc_y, y_pred)
        enc_X_T = [[enc_X[j][i] for j in range(len(enc_X))]
                   for i in range(len(enc_X[0]))]
        # print(f"y_pred shape: {len(y_pred)}x{len(y_pred[0])}")
        # print(f"X Shape: {len(enc_X)}x{len(enc_X[0])}")
        # print(f"X_T shape: {len(enc_X_T)}x{len(enc_X_T[0])}")
        # print(f"Error shape: {len(error)}x{len(error[0])}")

        grad = matrix_product(HE, enc_X_T, error)
        grad = matrix_scalar_mul(HE, grad, 2/len(enc_X))
        # print(f"Gradient shape: {len(grad)}x{len(grad[0])}")
        enc_theta = matrix_addition(HE, enc_theta, grad)
        # print(f"Theta shape: {len(enc_theta)}x{len(enc_theta[0])}")
...
# result_matmul = [[HE.decryptFrac(val)[0] for val in row]
#                  for row in enc_result_matmul]
...
# result_scalar = [[HE.decryptFrac(val)[0] for val in row]
#                  for row in enc_result_scalar]
...
# result_sub = [[HE.decryptFrac(val)[0] for val in row]
#               for row in enc_result_sub]
...
# result_add = [[HE.decryptFrac(val)[0] for val in row]
#               for row in enc_result_add]
...
# result_square = [[HE.decryptFrac(val)[0] for val in row]
#                  for row in enc_result_square]
...
#                 raise ValueError(
#                     f"Dot product test failed for vectors of size {n}")
...
#                         result = [[HE.decryptFrac(val)[0] for val in row]
#                                   for row in enc_result]
#                         expected = np.dot(mat1, mat2)
#                         if not np.allclose(result, expected, rtol=1e-3, atol=1e-3):
#                             raise ValueError(
#                                 f"Matrix product test failed for matrices of size ({m},{n}) and ({n},{p})")
#                         pbar.update(1)
...
#                     if not np.allclose(result, expected, rtol=1e-3, atol=1e-3):
#                         raise ValueError(
#                             f"Matrix-scalar multiplication test failed for matrix of size ({m},{n})")
#                     pbar.update(1)
...
#                     if not np.allclose(result, expected, rtol=1e-3, atol=1e-3):
#                         raise ValueError(
#                             f"Matrix addition test failed for matrices of size ({m},{n})")
#                     pbar.update(1)
...
#                     if not np.allclose(result, expected, rtol=1e-3, atol=1e-3):
#                         raise ValueError(
#                             f"Matrix subtraction test failed for matrices of size ({m},{n})")
#                     pbar.update(1)
...
#             if not np.allclose(result, expected, rtol=1e-3, atol=1e-3):
#                 raise ValueError(
#                     f"Matrix square test failed for matrix of size ({n},{n})")
